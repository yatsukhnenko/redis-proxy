...
int *rp_listen(int *sockfd, in_addr_t address, in_port_t port)
{
    ...
    if(bind(s, (const struct sockaddr *)&addr, sizeof(addr)) < 0) {
        syslog(LOG_ERR, "bind at %s:%d - %s\n", __FILE__, __LINE__, strerror(errno));
        close(s);
        return NULL;
    }
    if(listen(s, RP_CONCURRENT_CONNECTIONS) < 0) {
        syslog(LOG_ERR, "listen at %s:%d - %s\n", __FILE__, __LINE__, strerror(errno));
        close(s);
        return NULL;
    }
    *sockfd = s;
...
int rp_recv(int sockfd, rp_buffer_t *buffer) {
    ...
    for(;;) {
        if(buffer->used + RP_BUFFER_SIZE > buffer->s.length) {
            buffer->s.length += RP_BUFFER_SIZE;
            buffer->s.data = realloc(buffer->s.data, buffer->s.length);
        }
